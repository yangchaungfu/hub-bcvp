...
def evaluate(model):
    ...
    category_count = [0] * 5
    for label in y:
        category_count[int(label.item())] += 1
    print("本次预测集中共有%d个0类样本，%d个1类样本，%d个2类样本，%d个3类样本，%d个4类样本" % 
          (category_count[0], category_count[1], category_count[2], category_count[3], category_count[4]))
    correct, wrong = 0, 0
    with torch.no_grad():
        y_pred = model(x)  # 模型预测 model.forward(x)
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            # 获取预测类别和真实类别
            predicted_class = torch.argmax(y_p).item()
            true_class = y_t.item()
