...
class TorchModel(nn.Module):
    def __init__(self, input_size, num_classes):
        ...
        self.loss = nn.CrossEntropyLoss()  # 损失函数改为交叉熵

    # 当输入真实标签，返回loss值；无真实标签，返回预测值
    def forward(self, x, y=None):
        x = self.linear(x)  # (batch_size, input_size) -> (batch_size, num_classes)
        if y is not None:
            return self.loss(x, y)  # 交叉熵损失直接接受未归一化的 logits
        else:
            return torch.softmax(x, dim=1)  # 输出概率分布


# 生成一个样本, 样本的生成方法，代表了我们要学习的规律
# 随机生成一个5维向量，最大的数字在哪一维，就属于哪一类
def build_sample():
    x = np.random.random(5)
    y = np.argmax(x)  # 最大值所在的维度作为类别
    return x, y
...
def build_dataset(total_sample_num):
    ...
    for i in range(total_sample_num):
        ...
        X.append(x)
        Y.append(y)
    return torch.FloatTensor(X), torch.LongTensor(Y)  # 标签改为 LongTensor
...
def evaluate(model):
    ...
    with torch.no_grad():
        y_pred = model(x)  # 模型预测
        y_pred_classes = torch.argmax(y_pred, dim=1)  # 取概率最大值对应的类别
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        ...
        print("输入：%s, 预测类别：%d, 概率分布：%s" % (vec, predicted_class, res.numpy()))  # 打印结果
