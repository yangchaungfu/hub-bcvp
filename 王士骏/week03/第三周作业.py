...
基于pytorch的网络编写
实现一个网络完成一个简单nlp任务
判断文本中某些特定字符出现在第几位就属于哪个分类，如果没有指定文字也单独归为一类
...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, hidden_size, sentence_length, vocab):
        ...
        self.activation = nn.functional.softmax  # softmax激活函数
        self.loss = nn.functional.cross_entropy  # loss函数采用交叉熵损失

    # 当输入真实标签，返回loss值；无真实标签，返回预测值
    def forward(self, x, y=None):
        x = self.embedding(x)  # (batch_size, sen_len) -> (batch_size, sen_len, vector_dim)
        _, x = self.layer(x)  # (batch_size, vector_dim, sen_len)->(batch_size, hidden_size, 1)
        x = x.squeeze()  # (batch_size, hidden_size, 1) -> (batch_size, hidden_size)
        x = self.classify(x)  # (batch_size, hidden_size) -> (batch_size, sen_len+1)
        y_pred = self.activation(x, dim=1)  # (batch_size, sen_len+1) -> (batch_size, sen_len+1)
        if y is not None:
...
# 指定字符('我')出现在哪个位置就属于哪个分类，如果不存在就属于第sentence_length个分类
...
def evaluate(model, vocab, sample_length):
    ...
    print("本次预测集中共有",end=':')
    for i in range(7):
        print("{0}个分类{1}的样本".format(torch.sum(y == i).item(), i), end=';')
    print()
    correct, wrong = 0, 0
    with torch.no_grad():
        y_pred = model(x)  # 模型预测
...
def predict(model_path, vocab_path, input_strings):
    ...
    for vec, res in zip(input_strings, result):
        print("输入：{0}, 预测类别：{1}, 概率值：{2}".format(vec, np.argmax(res), res))  # 打印结果


if __name__ == "__main__":
    main()
    test_strings = ["fngf我e", "wz你dfg", "我qwdeg", "n我kwww"]
    predict("model02.pth", "vocab.json", test_strings)
