...
class Evaluator:
    ...
    def write_stats(self, labels, pred_results, sentences):
        ...
        for true_label, pred_label, sentence in zip(labels, pred_results, sentences):
            ...
            # print(pred_entities)
            # print('=+++++++++')
            with open("result.txt",mode='a',encoding='utf8') as f:
                f.write("正确的实体："+str(true_entities.items())+'\n')
                f.write("预测的实体："+str(pred_entities.items())+'\n')
                f.write("===================\n")
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load("model_output/bert_epoch_10.pth")) #使用训练好的bert模型预测


    evl = Evaluator(Config,model,logger)
    evl.eval(1)