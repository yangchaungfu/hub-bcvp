...
class TorchModel(nn.Module):
    def __init__(self, input_size):
        ...
        self.linear = nn.Linear(input_size, input_size)  # 线性层, 输入是5维向量, 输出也是5维
    ...
    def forward(self, x, y=None):
        y_pred = self.linear(x)  # shape: [batch_size, input_size] -> [batch_size, input_size]
        if y is not None:
...
def build_dataset(total_sample_num, n):
    """生成训练数据集，即生成一批样本"""
    X = []  # shape: [total_sample_num, n]
    Y = []  # shape: [total_sample_num]     # 注意：这里的y是真实标签, 它不是one-hot的, 这样才符合交叉熵Loss函数的传参要求
...
def evaluate(model):
    ...
    test_sample_num = 100
    x, y = build_dataset(test_sample_num, 5)
...
def main():
    ...
    for epoch in range(epoch_num):
        ...
        for batch_index in range(train_sample // batch_size):
            ...
            x = train_x[batch_index * batch_size: (batch_index + 1) * batch_size]  # shape: [batch_size, input_size]
        ...
        avgLoss = np.mean(watch_loss)  # 本轮的平均Loss
        accuracy = evaluate(model)  # 测试模型的准确率
        log.append([accuracy, float(avgLoss)])
...
def predict(model_path, input_vec):
    ...
    with torch.no_grad():  # 不计算梯度
        result = model.forward(torch.FloatTensor(input_vec))  # 模型预测. result.shape: [batch_size, input_size]
...
if __name__ == "__main__":
    main()
    print("\n使用训练好的模型做预测...")
    test_vec = [[0.078886, 0.152275, 0.310123, 0.035017, 0.889843],
