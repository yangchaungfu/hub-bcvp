...
class TorchModel(nn.Module):
    def __init__(self, input_size):
        ...
        self.loss = nn.CrossEntropyLoss()  # loss函数采用CrossEntropy

    # 当输入真实标签，返回loss值；无真实标签，返回预测值
    def forward(self, x, y=None):
        out = self.linear(x)  # (batch_size, input_size) -> (batch_size, 1)
        y_pred = torch.softmax(out, dim=1)  # (batch_size, 1) -> (batch_size, 1)
        if y is not None:
...
# 随机生成一组5维向量，以最大值维度为类比
def build_dataset(data_size, need_y=True):
