...
class LanguageModel(nn.Module):
    def __init__(self, input_dim, vocab):
        ...
        self.encoder = BertModel.from_pretrained(r"E:\pycharmproject\大模型培训-八斗\week9 序列标注问题\bert-base-chinese", return_dict=False)

        # self.classify = nn.Linear(input_dim, len(vocab))
        self.classify1 = nn.Linear(768, len(vocab))
    ...
    def forward(self, x, y=None):
        ...
        y_pred = self.classify1(x)  # output shape:(batch_size, sen_len, vocab_size)
        if y is not None:
