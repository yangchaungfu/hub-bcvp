...
class GraphQA:
    def __init__(self):
        ...
        self.graph = Graph("bolt://localhost:7687", auth=("neo4j", "demo"))
...
if __name__ == "__main__":
    graph = GraphQA()
    res = graph.query("旺角卡门是哪位导演的作品")
    print(res)
    res = graph.query("忘情水的谱曲是谁")
    print(res)
    res = graph.query("一起走过的日子的填词是谁")
    print(res)
    res = graph.query("刘德华的星座")
    print(res)
    res = graph.query("刘德华的女儿是谁")
    print(res)
    res = graph.query("谁创办了映艺娱乐")
    print(res)
    res = graph.query("四大天王的成员有哪些")
    print(res)
