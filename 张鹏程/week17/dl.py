...
class DialogueSystem:
    ...
    def is_repeat_request(self, query):
        repeat_examples = ["再说一遍", "重复一下", "没听清楚", "复述一遍",
                           "你刚才说什么", "再说一次", "没听到", "麻烦再说一遍"]
        return any(self.calucate_sentence_score(query, ex) > 0.3 for ex in repeat_examples)
    ...
    def run(self, query, memory):
        ...
        '''
        memory["query"] = query
        if self.is_repeat_request(query) and "last_response" in memory:
            memory["response"] = memory["last_response"]
            return memory
        memory = self.nlu(memory)
        if memory["intent_score"] < 0.1 and memory.get("policy") != "request":
            memory["response"] = memory.get("last_response", "我没听懂，请再说一遍")
            return memory
