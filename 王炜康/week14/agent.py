...
def get_recommendations(product_ids: list, insured_amount: int, years: int, age: int):
    ...
    """
    # 获取保险产品列表
    products = json.loads(get_insurance_products())

    # 获取保险产品详情
    product_details = [json.loads(get_product_detail(product_id)) for product_id in product_ids]

    # 获取保险产品的保费
    premiums = [json.loads(calculate_premium(product_id, insured_amount, years, age)) for product_id in product_ids]
    sum_premium = sum([premium["annual_premium"] for premium in premiums])
    # 排序
    premiums.sort(key=lambda x: x["annual_premium"])
    result = {
        "products": products,
        ...
        "recommend": [
            {
                ...
                "ratio": round(premium["annual_premium"] / sum_premium, 2)
...
def run_agent(user_query: str, api_key: str = None, model: str = "qwen-plus-2025-12-01"):
    """
    运行Agent，处理用户查询
...
if __name__ == "__main__":
    ...
    #实例 6：获取各个保险产品的投保比例
    run_agent("我需要买保险产品，保额是100万，我35岁，保20年，如果有多个产品，请告诉我各个产品的投保比例", model="qwen-plus")
