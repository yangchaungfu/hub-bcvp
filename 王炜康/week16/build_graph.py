...
#读取实体-关系-实体三元组文件
with open("triplets_head_rel_tail_james.txt", encoding="utf8") as f:
    for line in f:
        line = line.strip()
        if not line:  # 跳过空行
            continue
...
#读取实体-属性-属性值三元组文件
with open("triplets_enti_attr_value_james.txt", encoding="utf8") as f:
    for line in f:
        line = line.strip()
        if not line:  # 跳过空行
            continue
...
with open("kg_schema_james.json", "w", encoding="utf8") as f:
