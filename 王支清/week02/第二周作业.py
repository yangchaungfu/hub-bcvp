...
规律：x是一个5维向量，如果第一个数最大，则为第一类，以此类推，共5类。
...
# 随机生成一个5维向量，如果第一个值最大，认为是第一类，以此类推，共5类
def build_sample():
...
def predict(model_path, input_vec):
    ...
    with torch.no_grad():  # 不计算梯度
        ...
        probabilities = torch.softmax(result, dim=1)  # 应用softmax得到概率
        predicted_classes = torch.argmax(result, dim=1)  # 得到预测的类别
    for vec, pred_class,probs in zip(input_vec, predicted_classes,probabilities):
        print("输入：%s, 预测类别：%d" % (vec, pred_class))
        print("各类别概率：", [f"{p:.4f}" for p in probs])
        print("---")
