...
class DiyBertModel(nn.Module):
    ...
    def forward(self, num_layers, num_heads, x_word_token, x_segment_token, x_position_token, mlm_label=None, nsp_label=None):
        # embedding层
...
def corpus_to_token(sample_size, corpus_path):
    ...
    with open(corpus_path, "r", encoding="utf8") as f:
        for line in f:
            ...
            parts = line.split()
            tokens = tokenizer(parts[0],parts[1], padding="max_length", truncation=True, max_length=32)
...
def train(corpus_dict, vocab_len):
    ...
    for i in range(epoch_num):
        ...
        for j in range(len(corpus_dict) // batch_size):
            ...
            x_word_token, x_segment_token, x_position_token, mlm_label, nsp_label = build_batch_sample(j, corpus_dict, batch_size)
            loss = model(num_layers, num_heads, x_word_token, x_segment_token, x_position_token, mlm_label, nsp_label)
            loss.backward()
            optim.step()
            optim.zero_grad()
