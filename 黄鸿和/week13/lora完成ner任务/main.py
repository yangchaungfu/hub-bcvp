...
def main(config):
    ...
    if tuning_tactics == "lora_tuning":
        ...
        # TOKEN_CLS：Token Classification（序列标注），如 NER、词性标注
        # SEQ_CLS：Sequence Classification（句子分类），如情感分析、文本分类
    ...
    for epoch in range(config["epoch"]):
        ...
        for index, batch_data in enumerate(train_data):
            ...
            if isinstance(outputs, tuple):
                logits = outputs[0]  # 如果是tuple，取第一个元素
            else:
