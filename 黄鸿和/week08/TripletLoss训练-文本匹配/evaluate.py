...
class Evaluator:
    ...
    def knwb_to_vector(self):
        ...
        for standard_question_index, question_ids in self.train_data.dataset.knwb.items():
            ...
            # question_ids 是该标准问题下的所有问题列表
            for question_id in question_ids:
                #记录问题编号到标准问题标号的映射，用来确认答案是否正确
                # question_index_to_standard_question_index的key是问题在knwb_vectors中的索引
                # len(self.question_ids)即为当前问题在knwb_vectors中的索引
                self.question_index_to_standard_question_index[len(self.question_ids)] = standard_question_index
                # print('self.question_index_to_standard_question_index:', self.question_index_to_standard_question_index)
                self.question_ids.append(question_id)
        with torch.no_grad():
            # torch.stack将多个tensor沿着新的维度拼接起来，形成一个新的tensor
