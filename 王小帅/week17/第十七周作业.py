...
class DialogueSystem:
    ...
    def nlu(self, memory):
        repeat_keywords = ["什么", "什么意思", "没看懂", "详细说下"]
        if query in repeat_keywords and "final_response" in memory:
            memory["response"] = memory["final_response"]
            memory["final_response"] = None
            return memory
    ...
    def dpo(self, memory):
        if memory["require_slot"] is None:
            ...
            hit_node = memory["hit_node"]
            node_info = self.all_node_info[hit_node]
            memory["available_nodes"] = node_info.get("childnode", ["已完成订单，可重复选择"])
        else:
            # 有欠缺的槽位
    ...
    def nlg(self, memory):
        ...
        memory["final_response"] = memory["response"]
        return memory
...
if __name__ == '__main__':
    ...
    while True:
        ...
        print(response)
        if memory['response'] == '已结束订单，可重新选择。' :
