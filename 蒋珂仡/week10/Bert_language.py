...
class LanguageModel(nn.Module):
    def __init__(self, vocab):
        super(LanguageModel, self).__init__()
        self.hidden_size = 768  # BERT的标准维度


        config = BertConfig(
            vocab_size=len(vocab),
            hidden_size=self.hidden_size,
            num_hidden_layers=6,  # 层数，为了跑得快一点，我减半了（标准是12）
            num_attention_heads=12,  # 注意力头数
            is_decoder=True,  # <--- 核心修改：变身成 GPT 模式
        ...
        self.classify = nn.Linear(self.hidden_size, len(vocab))
    ...
    def forward(self, x, y=None):
        ...
        # 经过分类层
        y_pred = self.classify(sequence_output)  # shape: (batch_size, sen_len, vocab_size)
