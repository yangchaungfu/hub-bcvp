...
class SentenceEncoder(nn.Module):
    ...
    #输入为问题字符编码
    def forward(self, x):
        x = self.embedding(x)   #（30,20,128）
        #使用lstm
        # x, _ = self.lstm(x)
        #使用线性层
        x = self.layer(x)          #（30,20,128）
        x = nn.functional.max_pool1d(x.transpose(1, 2), x.shape[1]).squeeze()     #（30,128）
        return x
...
class SiameseNetwork(nn.Module):
    def __init__(self, config):
        ...
        self.sentence_encoder = SentenceEncoder(config) #实例化句子编码器
    ...
    def cosine_triplet_loss(self, a, p, n, margin=None):   #三元组余弦损失函数
        # 计算a和p的余弦距离
