...
class BertModelComplete(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None):
        ...
        # Apply pooler to get pooled output
...
def main():
    ...
    # 创建我们的BERT模型并手动设置权重
    model = BertModelComplete(num_hidden_layers=12)  # BERT-base有12层
    load_weights_from_state_dict(model, state_dict)  # 手动加载权重
    # model.load_state_dict(state_dict, strict=False)  # strict=False因为我们省略了一些权重
    model.eval()
