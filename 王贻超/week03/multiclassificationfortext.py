...
class TorchModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        x, _ = self.layer(x)                        # (batch_size, sen_len, vector_dim) -> (batch_size, sen_len, hidden_size)
...
def build_sample(vocab, sentence_length):
    ...
    try:
        y = x.index("a")  # 如果找到"a"，返回其索引位置
    except ValueError:
        y = sentence_length  # 如果没有找到"a"，返回sentence_length
...
def predict(model_path, vocab_path, input_strings):
    char_dim = 20
    sentence_length = 6
    hidden_size = 64  # 添加hidden_size参数
    vocab = json.load(open(vocab_path, "r", encoding="utf8"))
    model = build_model(vocab, char_dim, sentence_length, hidden_size)  # 添加hidden_size参数
    model.load_state_dict(torch.load(model_path))
    x = []
    for input_string in input_strings:
        x.append([vocab.get(char, vocab['unk']) for char in input_string])  # 处理未知字符
    model.eval()
    with torch.no_grad():
        result = model.forward(torch.LongTensor(x))
        classes = torch.argmax(result, dim=1)
    for i, input_string in enumerate(input_strings):
        predicted_pos = classes[i].item()
        probability = result[i][predicted_pos].item()
        print("输入：%s, 预测类别：%d, 概率值：%f" % (input_string, predicted_pos, probability))



if __name__ == "__main__":
    main()
    test_strings = ["fnavf我", "awzadf", "rqwdea", "n我kwww"]
