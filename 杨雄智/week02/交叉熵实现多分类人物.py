...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        y_pred_class = torch.argmax(y_pred, dim=1)  # 得到预测类别
...
def main():
    ...
    print("训练标签形状：", train_y.shape)  # 应该输出 torch.Size([5000])
    # 训练过程
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        pred_class = torch.argmax(res).item()  # 单个样本的预测类别（0~4）
        print(f"输入：{vec}，预测最大数字所在维度：{pred_class}")
