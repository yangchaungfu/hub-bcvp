...
class SiameseNetwork(nn.Module):
    ...
    def forward(self, anchor, positive=None, negative=None, target=None, margin=None):
        # 三元组输入模式：同时传入 anchor, positive, negative
        ...
        # 原有 pairwise 模式：传入两个句子计算损失或距离
        elif positive is not None:
            vector1 = self.sentence_encoder(anchor)  # anchor 作为第一个句子
