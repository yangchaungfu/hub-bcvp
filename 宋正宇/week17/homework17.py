...
class DialogueSystem:
    ...
    def save_dialogue_turn(self, query, response, memory):
        """保存单轮对话记录，用于重听"""
        turn = {
            "round": len(self.dialogue_history) + 1,
            "user_query": query,
    ...
    def replay_dialogue(self, round_num=None):
        """
        重听功能：可指定轮次或全部回放
        :param round_num: 要回放的轮次，None则回放全部
        :return: 格式化的重听结果
        """
        ...
        replay_content = []
        if round_num:
            if 1 <= round_num <= len(self.dialogue_history):
                turn = self.dialogue_history[round_num - 1]
                replay_content.append(f"第{turn['round']}轮对话：")
                replay_content.append(f"用户：{turn['user_query']}")
                replay_content.append(f"系统：{turn['system_response']}")
            else:
                return f"无效轮次，当前仅{len(self.dialogue_history)}轮对话"
        else:
            replay_content.append("===== 全部对话记录 =====")
            for turn in self.dialogue_history:
                replay_content.append(f"\n第{turn['round']}轮：")
                replay_content.append(f"用户：{turn['user_query']}")
                replay_content.append(f"系统：{turn['system_response']}")
    ...
    def run(self, query, memory):
        # 识别重听指令
        if query.strip() in ["重听", "回放", "查看历史"]:
            memory["response"] = self.replay_dialogue()
            return memory
        '''
        query: 用户输入
        memory: 用户状态
        ...
        # 保存对话记录
        self.save_dialogue_turn(query, memory["response"], memory)
        return memory
