...
class StackGatedCNN(nn.Module):
    def __init__(self, config):
        ...
        # ModuleList类用于存放多个模型，取用时类似为一个列表
        self.gcnn_layers = nn.ModuleList(
            GatedCNN(config) for _ in range(self.num_layers)
        )
        self.ff_linear_layers1 = nn.ModuleList(
            nn.Linear(self.hidden_size, self.hidden_size) for _ in range(self.num_layers)
        )
        self.ff_linear_layers2 = nn.ModuleList(
            nn.Linear(self.hidden_size, self.hidden_size) for _ in range(self.num_layers)
        )
        self.ln_after_gcnn = nn.ModuleList(
            nn.LayerNorm(self.hidden_size) for _ in range(self.num_layers)
        )
        self.ln_after_ff = nn.ModuleList(
            nn.LayerNorm(self.hidden_size) for _ in range(self.num_layers)
        )
...
class BertMidLayer(nn.Module):
    ...
    def forward(self, x):
        layer_states = self.bert(x)[2]   # 13层隐藏状态的元组，对应BERT从词嵌入层在内的所有层状态
