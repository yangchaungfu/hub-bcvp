...
class Evaluator:
    ...
    def write_stats(self, labels, pred_results, sentences):
        # 处理新的模型输出格式
        if hasattr(pred_results, 'logits'):
            pred_results = pred_results.logits
        elif isinstance(pred_results, tuple):
            # 对于返回元组的情况，通常第一个元素是logits
            pred_results = pred_results[0]
        
        # 确保 pred_results 的维度正确
        if len(pred_results.shape) == 3:
            # 如果是三维张量，进行argmax操作
            pred_results = torch.argmax(pred_results, dim=-1)
        
        # 确保长度一致
        # 这里我们使用 labels 的长度作为基准，因为 sentences 可能会因为批次大小的原因而长度不同
        min_len = min(len(labels), len(pred_results), len(sentences))
