...
规律：x是一个5维向量，最大值所在的索引位置为类别标签(0-4)
"""
...
def predict(model_path, input_vec):
    ...
    for vec, pred_class, prob in zip(input_vec, predicted_classes, probabilities):
        true_class = np.argmax(vec)  # 真实类别
        print("输入：%s" % vec)
        print("真实类别：%d, 预测类别：%d" % (true_class, pred_class.item()))
        print("各类别概率：%s" % prob.numpy())
        print("---")

if __name__ == "__main__":
    main()
    test_vec = [[0.07889086,0.15229675,0.31082123,0.03504317,0.88920843],  # 类别4
                [0.74963533,0.5524256,0.95758807,0.95520434,0.84890681],   # 类别2
                [0.00797868,0.67482528,0.13625847,0.34675372,0.19871392],  # 类别1
                [0.09349776,0.59416669,0.92579291,0.41567412,0.1358894]]   # 类别2
