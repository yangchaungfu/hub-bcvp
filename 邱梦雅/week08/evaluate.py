...
class Evaluator:
    ...
    #将知识库中的问题向量化，为匹配做准备
    #每轮epoch训练的模型参数不一样，生成的向量也不一样，所以需要每轮测试都重新进行向量化
    ...
    def eval(self, epoch):
        ...
        for index, batch_data in enumerate(self.valid_data):
            ...
            with torch.no_grad():
                # input_id (32, 20) (batch_size, max_length)
                # test_question_vectors (32, 28) (batch_size, hidden_size)
    ...
    def write_stats(self, test_question_vectors, labels):
        ...
        for test_question_vector, label in zip(test_question_vectors, labels):
            ...
            # torch.mm 只适用于二维张量批量乘积。如果输入是更高维的张量，会抛出错误。对于更高维的矩阵乘法，可以使用 torch.matmul
