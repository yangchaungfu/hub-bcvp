...
class TorchModel(nn.Module):
    ...
    def forward(self, x, target=None):
        ...
        predict = self.classify(x) # output:(batch_size, sen_len, hidden_size) -> (batch_size * sen_len, num_tags)
        ...
        if target is not None:
            ...
            if self.use_crf:
                return self.crf_layer.decode(predict)  # 解码出来的是一个序列 (batch_size, sen_len) （在evaluate的时候要想办法包标签序列还原为文本段）
            else:
                return predict
