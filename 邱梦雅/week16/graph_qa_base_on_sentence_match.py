...
class GraphQA:
    ...
    def get_combinations(self, cypher_check, info):
        ...
        for key, required_count in cypher_check.items():
            # tertools.combinations(iterable, r)。它的作用是从 iterable（这里是 info[key]）中返回长度为 r（这里是 required_count）的子序列。
            slot_values.append(itertools.combinations(info[key], required_count))   #
        # 计算输入序列的笛卡尔积。简单来说，就是从第一类组合中拿一种方案，从第二类组合中拿一种方案，……，直到最后一类，将它们拼成一个大的元组。
    ...
    def cypher_match(self, sentence, info):
        ...
        # print("templet_cypher_pair: ", templet_cypher_pair)
        for i, templet in enumerate(templet_cypher_pair):
            print(f"{i + 1}: {templet}")
        result = []
...
if __name__ == "__main__":
    ...
    # print(res)
    res = graph.query("Susan说的编曲是谁")
    print(res)
    res = graph.query("望春风的编曲是谁")
    print(res)
    res = graph.query("陶喆的星座是什么")
    print(res)
    res = graph.query("陶喆的血型是什么")
    print(res)
    res = graph.query("陶喆的身高")
    print(res)
    res = graph.query("陶喆和加利福尼亚大学洛杉矶分校是什么关系")
    print(res)
