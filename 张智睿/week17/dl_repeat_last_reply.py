...
def jaccard_similarity(a: str, b: str) -> float:
    set_a = set(a.split())  # 将句子拆分为词集
    set_b = set(b.split())  # 将句子拆分为词集
...
class DialogueSystem:
    ...
    def nlu(self, memory):
        # 保持 memory 是字典类型，并包含必要的键
        if not isinstance(memory, dict):
            raise ValueError("memory 应该是字典类型。")
        ...
        # 扩展“重听”和“重看”关键词库
        repeat_keywords = [
            "我没听清", "你刚刚说啥", "请再说一遍", "请重复一遍", "请大声点",
            "能再说一次吗", "你刚才说什么", "我没有听清楚", "再说一遍好吗", "请讲慢点",
            "说清楚一点", "我没听懂", "能重复一遍吗", "请再说一遍", "再讲一遍",
            "刚才说的能再说一次吗", "可以重复一下吗", "能不能再讲一遍", "能不能再说一遍",
            "能再说一遍吗", "请再说一遍", "麻烦再说一遍",

            # "重看"相关表达
            "我没看清", "你刚刚说的是啥", "再看一遍", "能再看一遍吗", "可以再看一遍吗",
            "我没看明白", "请再展示一遍", "能不能重放一遍", "再演示一遍", "请再给我看看",
            "能不能再看一遍", "再给我展示一遍", "能再给我看一遍吗", "请再给我看一次",
            "再放一遍", "能再重复一遍吗", "能看清楚点吗", "再看一下"
        ]

        # 判断用户输入是否与重听、重看相关的关键词相似
        threshold = 0.6  # 设定相似度阈值
        for keyword in repeat_keywords:
            score = jaccard_similarity(memory['query'], keyword)
            if score >= threshold:
                return "repeat_last_reply", {}  # 返回“重听”或“重看”指令
    ...
    def run(self, query, memory):
        ...
        # 确保 memory 是字典结构
        if not isinstance(memory, dict):
            raise ValueError("memory 应该是一个字典类型。")
        ...
        # 存储最近一次的回复
        self.last_reply = memory['response']
